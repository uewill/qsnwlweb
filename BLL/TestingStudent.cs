...
namespace TFXK.BLL
{
	...
	{
		private readonly ITestingStudent dal=DataAccess.CreateTestingStudent();
		...
		/// 获得数据列表
		/// </summary>
		public List<TFXK.Model.TestingStudent> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<TFXK.Model.TestingStudent> DataTableToList(DataTable dt)
		{
			List<TFXK.Model.TestingStudent> modelList = new List<TFXK.Model.TestingStudent>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				TFXK.Model.TestingStudent model;
        ...
        {
            return GetList("[tb_TestingStudent]", PageSize, PageIndex, where, "id desc", out rowCount);
        }
