...
{
	/// <summary>
	/// 业务逻辑类UserJiamenBLL 的摘要说明。
	/// </summary>
	public class UserJiamenBLL
	{
		private readonly IUserJiamenDAL dal=DataAccess.CreateUserJiamenDAL();
        private readonly DataPageListBLL bll = new DataPageListBLL();
		public UserJiamenBLL()
		{}
		#region  成员方法
		...
		/// 获得数据列表
		/// </summary>
		public List<TFXK.Model.UserJiamen> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<TFXK.Model.UserJiamen> DataTableToList(DataTable dt)
		{
			List<TFXK.Model.UserJiamen> modelList = new List<TFXK.Model.UserJiamen>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				TFXK.Model.UserJiamen model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new TFXK.Model.UserJiamen();
        ...
        {
            return bll.GetList("tb_UserJiamen", PageSize, PageIndex, strWhere, "id desc", out rowCount);
        }
